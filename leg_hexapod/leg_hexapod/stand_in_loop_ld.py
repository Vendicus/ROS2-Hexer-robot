...
class stand_loop(Node):
    def __init__(self, angle_start_x, angle_start_y, angle_start_z, Z):
        super().__init__('stand_loop_ld')
        second_cycle_ld = MutuallyExclusiveCallbackGroup()
        self.publisher_LD = self.create_publisher(ThetasAll, 'anglesDLLegList', 10, callback_group=second_cycle_ld)
        self.publisher_LD_work = self.create_publisher(Semafor, 'Semafor', 3)
        self.timer = self.create_timer(0.01, self.stand_in_loop_ld)
