...
class stand_loop(Node):
    def __init__(self, angle_start_x, angle_start_y, angle_start_z, Z):
        super().__init__('stand_loop_rd')
        first_cycle_rd = MutuallyExclusiveCallbackGroup()

        self.publisher_RD = self.create_publisher(ThetasAll, 'anglesDRLegList', 10, callback_group=first_cycle_rd)
        self.publisher_RD_work = self.create_publisher(Semafor, 'Semafor', 3)
        self.timer = self.create_timer(0.01, self.stand_in_loop_rd)
