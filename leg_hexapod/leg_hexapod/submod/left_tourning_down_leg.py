...
def tourning(tourning_value, speed):
    ...
    if tourning_value >= 0.0 :
        ...
        if speed >= 0:
                for i in reversed(range(0, angle2.size) ):
                        y4_tourning.append(np.float16(  -( r3 * np.cos(np.deg2rad(angle2[i])) + tourning_value) )) 
                        x4_tourning.append(np.float16( r3 * np.sin(np.deg2rad(angle2[i]))))  
        else:
                for i in range(0, angle2.size):
                        y4_tourning.append(np.float16(  -( r3 * np.cos(np.deg2rad(angle2[i])) + tourning_value) )) 
                        x4_tourning.append(np.float16( r3 * np.sin(np.deg2rad(angle2[i]))))
    ...
    r_y4_tourning = regulate(y4_tourning)

    for i in range(len(r_y4_tourning)):
        yield x4_tourning[i], r_y4_tourning[i]