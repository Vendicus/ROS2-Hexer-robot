...
def tourning(tourning_value, speed):
    ...
    if tourning_value >= 0.0 :
        ...
        if speed >= 0:
                for i in reversed(range(0, angle1.size) ):
                        y3_tourning.append(np.float16( - ( r3 * np.cos(np.deg2rad(angle1[i])) + tourning_value) )) 
                        x3_tourning.append(np.float16( r3 * np.sin(np.deg2rad(angle1[i]))))
        else:
                for i in range(0, angle1.size) :
                        y3_tourning.append(np.float16( - ( r3 * np.cos(np.deg2rad(angle1[i])) + tourning_value) )) 
                        x3_tourning.append(np.float16( r3 * np.sin(np.deg2rad(angle1[i]))))  
    else :
    ...
    r_y3_tourning = regulate(y3_tourning)

    for i in range(len(r_y3_tourning)):
        yield x3_tourning[i], r_y3_tourning[i]
