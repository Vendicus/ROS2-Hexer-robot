...
def tourning(tourning_value, speed):
    ...
    theta = 2 * np.degrees(np.arcsin(prosta / (2.0*r1 if tourning_value >= 0.0 else 2.0*r2)))
    ...
    if speed >= 0:
        for i in range(0, angle.size) :
                y2_tourning = np.float16( r2 * np.cos(np.deg2rad(angle[i])) + tourning_value - 80.0)
                x2_tourning = np.float16( - r2 * np.sin(np.deg2rad(angle[i])) )

                yield x2_tourning, y2_tourning
    else:
        for i in reversed (range(0, angle.size) ):
                y2_tourning = np.float16( r2 * np.cos(np.deg2rad(angle[i])) + tourning_value - 80.0)
                x2_tourning = np.float16( - r2 * np.sin(np.deg2rad(angle[i])) )
